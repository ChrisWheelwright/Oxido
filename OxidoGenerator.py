...
def generate_html():
    try:
        ...
        label_status.config(text="Przetwarzanie... Proszę czekać.")
        ...
        label_status.config(text="Gotowe! Plik zapisano.")
    except Exception as e:
        messagebox.showerror("Error", f"Coś poszło nie tak: {e}")
        label_status.config(text=f"Błąd: {e}")
    finally:
...
# Function to run generate_html in a separate thread
def generate_html_thread():
...
button_generate = tk.Button(app, text="Generuj HTML", command=generate_html_thread, height=3, width=20)
